...
def get_noise(data, noise_type):
    if noise_type == "gaussian":
        """
        samples Gaussian noise according to the list stds, add it to data and returns the noisy data
        """
        std = [0.08, 0.12, 0.14, 0.18, 0.26, 0.38, 0.42]
        c = np.random.choice(std, data.shape[0], replace=True)
...
class BoxTransformer(nn.Module):
    ...
    def __init__(self, d_model=32, nhead=8, num_encoder_layers=2, dim_feedforward=2048, dropout=0.1,
        ...
        self.output_embedding_layer_temporal = torch.nn.Linear(d_model, self.feat_len)
    ...
    def forward(self, trajectory):
        ...
        :return:(f_last,B,2)
        """
        # noise = get_noise((trajectory.shape[1], 16), 'gaussian')
        if self.encoder is None:
            trajectory_out = trajectory[-1]
        else:
            # noise_to_cat = [noise.repeat(x.shape[0], 1) for x in trajectory]
            # trajectory_wnoise = torch.stack([torch.cat((trajectory[i], noise[i]), dim=1) for i in range(len(trajectory))],0)
            # trajectory_in = self.dropout(self.relu(self.input_embedding_layer_temporal(trajectory)))
            trajectory_in = self.dropout(self.relu(self.input_embedding_layer_temporal(trajectory)))
            trajectory_in_wnoise = get_noise(trajectory_in, 'gaussian')
            memory = self.encoder(trajectory_in_wnoise)[-1]
            # memory_wnoise = torch.cat((memory, noise), dim=1)  # 模拟输出多样性
            # noise = self.noise_gen(memory.unsqueeze(0)).squeeze(-1).permute(2, 0, 1).squeeze(0)
            trajectory_out = self.output_embedding_layer_temporal(memory)
        return trajectory_out  # (B,4)
